let { assert, isArray, shape, transpose, copy, chop, distance, identity, dot, add, scale, pow } = require("js-math-tools")
...
function gramSchmidtOrthonormalize(x){
  ...
  assert(containsOnlyNumbers(x), "`gramSchmidtOrthonormalize` only works on matrices of numbers!")
  assert(shape(x).length === 2, "`gramSchmidtOrthonormalize` only works on matrices!")
  ...
  assert(chop(distance(identity(out.length), dot(out, outTranspose))) === 0, "The matrix produced by the `gramSchmidtOrthonormalize` function must be orthogonal!")
  return outTranspose
}
